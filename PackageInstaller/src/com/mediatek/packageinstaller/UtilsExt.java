...
public class UtilsExt {
    ...
    public static void initPermStateCache(AppPermissions appPermissions, List<String> revokedPerms) {
        ...
        for (AppPermissionGroup group : appPermissions.getPermissionGroups()) {
            for (Permission permission : group.getPermissions()) {
                if (!Utils.shouldShowPermission(group,
                        appPermissions.getPackageInfo().packageName)) {
                    continue;
                }
                int state = revokedPerms != null && revokedPerms.contains(permission.getName()) ?
                        PermissionState.STATE_DENIED : PermissionState.STATE_ALLOWED;
                Log.d(TAG,"permission.getName() = " + permission.getName()
                        + " state = " + state);
    ...
    public static void setPermGroupStateCache(AppPermissions appPermissions ,
            String permGroupName, boolean grant) {
        AppPermissionGroup group = appPermissions.getPermissionGroup(permGroupName);
        for (Permission permission : group.getPermissions()) {
             setPermStateCache(permission,
                    grant ? PermissionState.STATE_ALLOWED :
                        PermissionState.STATE_DENIED);
        }
    }

    public static void setPermStateCache(Permission permission, int state) {
        Log.d(TAG, "setPermStateCache(), permission: " + permission.getName() +
                ", state: " + state);
        if (mPermStateCache.get(permission.getName()) == null) {
    ...
    public static boolean isPermGrantedByCache(Permission permission) {
        ...
        return mPermStateCache.get(permission.getName()).getState()
