...
public final class AppPermissionsFragment extends SettingsWithHeader
    ...
    private void loadPreferences() {
        ...
        for (AppPermissionGroup group : mAppPermissions.getPermissionGroups()) {
            ...
            /// M: CTA requirement - permission control @{
            boolean isPlatform = manager.isPlatformPermissionGroup(
                     group.getDeclaringPackage(), group.getName());

            OnPreferenceChangeListener switchChangeListener = new OnPreferenceChangeListener() {
                @Override
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    if (preference instanceof MultiTargetSwitchPreference) {
                        AppPermissionsFragment.this.onPreferenceChange(preference, newValue);
                        updateSummaryForIndividuallyControlledPermissionGroup(
                                group, preference);
                        ((MultiTargetSwitchPreference) preference).setCheckedOverride(
            ...
            if (Utils.areGroupPermissionsIndividuallyControlled(getContext(), group.getName())) {
                ...
                /// M: CTA requirement - permission control @{
                // Listen the checked state change of the Switch View.
                if (manager.isCtaSupported()) {
                    preference.setSwitchChangeListener(switchChangeListener);
                }
                ///@}
        ...
        if (manager.isCtaSupported()) {
            ...
            manageAllPermsPreference.setOnPreferenceClickListener(
                new OnPreferenceClickListener() {
                    @Override
                    public boolean onPreferenceClick(Preference preference) {
                        Fragment frag = AllAppPermissionsFragment.newInstance(
                            getArguments().getString(Intent.EXTRA_PACKAGE_NAME), null);
                        getFragmentManager().beginTransaction()
                            .replace(android.R.id.content, frag)
                            .addToBackStack("AllPerms")
                            .commit();
            ...
            screen.addPreference(manageAllPermsPreference);
        }
        ///@}
