...
public class EventLogger {
    ...
    private static final List<String> CTA_ADDED_PERMISSIONS = Arrays.asList(
    ...
    private static int getBaseEventId(@NonNull String name) {
        ...
        if (CtaManagerFactory.getInstance().makeCtaManager().isCtaSupported()) {
            int ctaPermIndex = CTA_ADDED_PERMISSIONS.indexOf(name);
            if (ctaPermIndex != -1) {
                Log.i(LOG_TAG, "cta added permission " + name);
