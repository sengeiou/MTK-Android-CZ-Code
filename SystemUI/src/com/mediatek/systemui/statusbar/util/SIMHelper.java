...
public class SIMHelper {
    ...
    /**
     * Check the current sub id whether is match mixed volte condition to show disconnected icon
     * Condition:
     *  1. The main SIM should be C card
     *  2. And the second SIM also should be C card
     * @param subId current sub id
     * @return
     */
    public static boolean isSecondaryCSIMForMixedVolte(int subId) {
        boolean ret = false;
        int mainPhoneId = getMainPhoneId();
        int mainPhoneSubId = MtkSubscriptionManager.getSubIdUsingPhoneId(mainPhoneId);
        int currentSoltId = SubscriptionManager.getSlotIndex(subId);
        // if is secondary SIM
        if (mainPhoneId != currentSoltId) {
            // 1. check main SIM whether is C card
            if ((isCTCardType(mainPhoneId) || isCtSim(mainPhoneSubId))
                    // 2. check secondary SIM whether is C card
                    && (isCTCardType(currentSoltId) || isCtSim(subId))) {
                ret = true;
            }
        }
        if (FeatureOptions.LOG_ENABLE) {
            Log.d("@M_" + TAG, "isSecondaryCSIMForMixedVolte, ret = "
                    + ret + ", subId = " + subId);
        }
        return ret;
    }
