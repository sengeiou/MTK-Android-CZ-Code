...
public class KeyguardUpdateMonitor implements TrustManager.TrustListener {
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_TIME_TICK.equals(action)
                ...
                //20190827 pjz add
                checkRilDataModelIs3G(serviceState);
            }
        }
    };

    //20190827 pjz add when boot open close airmode for avoid Unicom can't go 4G S
    private boolean justOnceRebooted = true;
    private void checkRilDataModelIs3G(ServiceState serviceState){
       int state = serviceState.getDataRegState();
       int mRilDataRadioTechnology = serviceState.getRilDataRadioTechnology();
       Log.e(TAG, "state="+state + "  mRilDataRadioTechnology="+mRilDataRadioTechnology);
       if (justOnceRebooted && state == ServiceState.STATE_IN_SERVICE && mRilDataRadioTechnology != 14) {
           //3g HSPA 11  4g LTE 14
            justOnceRebooted = false;
            Log.e(TAG, "current is 3g model need setAirPlaneMode");

            setAirPlaneMode(true, 10);

            setAirPlaneMode(false, 700);
       }else{
            Log.e(TAG, "don't need handle airPlanMode");
       }
    }

    private void setAirPlaneMode(final boolean enable, long delayTimes){
        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.e(TAG, "setAirPlaneMode enable="+enable);
                        int airPlanMode  = android.provider.Settings.Global.getInt(mContext.getContentResolver(),
                           android.provider.Settings.Global.AIRPLANE_MODE_ON, 0);
                        Log.e(TAG, "airPlanMode="+airPlanMode);

                        android.provider.Settings.Global.putInt(mContext.getContentResolver(), 
                            android.provider.Settings.Global.AIRPLANE_MODE_ON, enable ? 1 : 0);
    ...
    //20190827 pjz add when boot open close airmode for avoid Unicom can't go 4G E
    ...
    private static final String ACTION_UNLOCK_SIM_LOCK
            = MtkTelephonyIntents.ACTION_UNLOCK_SIM_LOCK;
    ...
    private CharSequence getTelephonyHnbNameFrom(Intent intent) {
        final String hnbName = intent.getStringExtra(MtkTelephonyIntents.EXTRA_HNB_NAME);
        return hnbName;
    }
    ...
    private CharSequence getTelephonyCsgIdFrom(Intent intent) {
        final String csgId = intent.getStringExtra(MtkTelephonyIntents.EXTRA_CSG_ID);
        return csgId;
    }
