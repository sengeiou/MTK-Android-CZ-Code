...
static const char CS_BOOTANIMATION_FILE[] = "/data/local/bootanimation.zip";
...
BootAnimation::BootAnimation(sp<Callbacks> callbacks, bool bSetBootOrShutDown, bool bSetPlayMP3,bool bSetRotated)
    ...
    if (powerCtl.empty()) {
     ...
     mShuttingDown = true;//add sunritel
...
void BootVideoListener::notify(int msg, int ext1, int ext2, const Parcel *obj) {
    ALOGD("jzg...[MtkBootAnimation %s %d] msg=%d ext1=%d ext2=%d",__FUNCTION__,__LINE__, msg, ext1, ext2);
    if(msg == MEDIA_PLAYBACK_COMPLETE || msg == MEDIA_SEEK_COMPLETE) {
        mBootanim->setBootVideoPlayState(MEDIA_PLAYBACK_COMPLETE);
        ALOGD("jzg...[MtkBootAnimation %s %d] media player complete",__FUNCTION__,__LINE__);
    }
    if(msg == MEDIA_ERROR || msg == MEDIA_SKIPPED) {
        mBootanim->setBootVideoPlayState(MEDIA_ERROR);
        ALOGD("jzg...[MtkBootAnimation %s %d] media player error",__FUNCTION__,__LINE__);
    }
    if(msg == MEDIA_STARTED) {
        mBootanim->setBootAudioStarted();
        ALOGD("jzg...XXXXX[MtkBootAnimation %s %d] AudioStarted",__FUNCTION__,__LINE__);
    }
...
#endif
    if(obj == NULL){
        ALOGD("jzg...[MtkBootAnimation %s %d]obj is null \n",__FUNCTION__,__LINE__);
    }
}
...
void BootAnimation::setBootAudioStarted(void)
{
    ...
    bAudioStarted = true;
    ALOGD("jzg...[MtkBootAnimation %s %d]bAudioStarted=%d",__FUNCTION__,__LINE__, bAudioStarted);
    mCondition.signal();
}
...
@}  */
    ...
    if (/*!mShuttingDown*/bBootOrShutDown && encryptedAnimation &&
    ...
    for (const char* f : (/*!mShuttingDown*/bBootOrShutDown ? bootFiles : shutdownFiles)) {
        if (access(f, R_OK) == 0) {
            mZipFileName = f;
            return NO_ERROR;
        }
...
bool BootAnimation::threadLoop()
{
    ...
    */
    ...
    if (resourcePath != NULL) {
        ...
        ALOGD("jzg...media player is prepared mediastatus %d,NO_ERROR %d",mediastatus,NO_ERROR);
        if (mediastatus == NO_ERROR) {
            ALOGD("jzg...mediaplayer->start");
            mediastatus = mediaplayer->start();
        }

    }else{
        bPlayMP3 = false;
        ALOGE("jzg...bPlayMP3: %d", bPlayMP3);
    }
    ...
    ALOGD("jzg...mCondition.waitRelative mediastatus %d,NO_ERROR %d bPlayMP3 %d bAudioStarted %d",mediastatus,NO_ERROR,bPlayMP3,bAudioStarted);
    ...
    if (resourcePath != NULL) {
        if (mediastatus == NO_ERROR) {
            ALOGD("jzg...mediaplayer was stareted successfully, now it is going to be stoped");
            mediaplayer->stop();
            mediaplayer->disconnect();
            mediaplayer.clear();
        }
        ALOGD("jzg...mediastatus %d NO_ERROR %d",mediastatus,NO_ERROR);
    }
    else
    	ALOGD("jzg...resourcePath is null");
...
{
    if (bBootOrShutDown){//pjz
        static const char* userBootFiles[] = {CS_BOOTANIMATION_FILE, ""};
        clogd("[MtkBootAnimation %s %d]",__FUNCTION__,__LINE__);
        for (const char* f : userBootFiles) {
            if (access(f, R_OK) == 0) {
                mZipFileName = f;
                clogd("CS_BOOTANIMATION_FILE done....");
            }
            if (access(f, F_OK) == 0) {
                clogd("CS_BOOTANIMATION_FILE exts....");
            }
        }
    }
...
const char* BootAnimation::initAudioPath() {
    ALOGE("jzg...initAudioPath: bPlayMP3 = %d", bPlayMP3);
    if (!bPlayMP3) {
    ...
    ALOGD("jzg... MTK_CARRIEREXPRESS_PACK: YES");
    ALOGD("jzg...enter initAudioPath while loop read property");
        for(int j=0;j<50;j++) {
    ...
    if (property_get(GLOBAL_DEVICE_BOOTANIM_OPTR_NAME,OPTR, NULL) > 0) {
        for(int i=0;i<PATH_COUNT_USP;i++) {
            ...
            ALOGD("jzg...[MtkBootAnimation %s %d]use operator = %s",__FUNCTION__,__LINE__,OPTR);
            ...
            if (access(AudioFileName, F_OK) == 0) {
                ALOGD("jzg...initAudioPath: audio path = %s",AudioFileName);
    ...
    ALOGD("jzg...Sim Optr is null");
    return NULL;
#else

    for (int i = 0; i < PATH_COUNT; i++) {
        if (access(mAudioPath[index][i], F_OK) == 0) {
            ALOGD("jzg...initAudioPath: audio path = %s", mAudioPath[index][i]);
...
void BootAnimation::initBootanimationZip() {
    ...
    String8     ZipFileName;
    ALOGD("jzg... MTK_TER_SERVICE: check");
#ifdef MTK_TER_SERVICE
    ...
    if ((access(CUSTOM_REGIONALPHONE_DB, F_OK) == 0)||(access(SYSTEM_REGIONALPHONE_DB, F_OK) == 0)) {
        ALOGD("jzg...regionalphone.db check OK");
